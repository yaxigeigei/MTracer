classdef KilosortResult < MTracer.SortingResult
    ...
    methods
        ...
        function ImportData(this, ksFolder, chanMapFile)
            ...
            opsFileMat = fullfile(ksFolder, 'ops.mat');
            opsFileNpy = fullfile(ksFolder, 'ops.npy');
            
            if exist(rezFile, 'file')
                % From rez.mat file generated by Kilosort 2.5 and earlier
                this.LoadRez(rezFile);
                
            elseif exist(opsFileMat, 'file')
                % From ops.mat file generated by Kilosort 2.5 run in spikeinterface
                load(opsFileMat, 'ops');
            ...
            elseif exist(opsFileNpy, 'file')
                % From ops.npy file generated by Kilosort 4
                np = py.importlib.import_module('numpy');
                ops = np.load(opsFileNpy, allow_pickle=true).item();
                
                ops = struct(ops);
                fns = fieldnames(ops);
                for i = 1 : numel(fns)
                    ops.(fns{i}) = MUtil.ConvertNpyTypes(ops.(fns{i}));
                end
                ...
                error("Cannot find supported files with necessary metadata.");
            end
        ...
        function bkFolder = ExportData(this, ksFolder)
            % Export the clustering results to specified Phy files in the Kilosort output folder.
